          ...
          child: data.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) => Card(
                    color: Theme.of(
                      context,
                    ).cardColor.withValues(alpha: uiOpacity),
  ...
  Widget _buildSearchResults() {
    ...
    sections.forEach((key, sec) {
      for (var it in DataManager.getItems(key)) {
        if (_normalize(it['title'].toString()).contains(normalizedQuery) ||
            _normalize(it['content'].toString()).contains(normalizedQuery)) {
          results.add({
            'section': sec['title'],
            'title': it['title'],
            'content': it['content'],
